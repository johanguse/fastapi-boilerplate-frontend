...
export function SocialLogin({ className, redirectUrl }: SocialLoginProps) {
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      try {
        ...
        // Failed to fetch providers - will show no social login options
      }
    }
  ...
  const handleSocialLogin = async (providerId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/auth/oauth/${providerId}/authorize?${params}`)
      
      if (!response.ok) {
        throw new Error(`Failed to get authorization URL for ${providerId}`)
      }
      ...
      alert(t('auth.socialLogin.error.description', { provider: providerId }))
    } finally {
        ...
        {enabledProviders.map((provider) => {
          ...
          return (
            <Button
              ...
              disabled={loading[provider.id] || isLoading}
              className={cn(
            ...
            >
              ...
              <span className="ml-2">
                {t('auth.socialLogin.continueWith', { provider: provider.name })}
              </span>
            </Button>
          )
        })}
