...
export function Dashboard({ userName }: { userName: string }) {
  ...
  return (
    <div>
      ...
      <p>{t('dashboard.lastLogin', { date: new Date().toLocaleDateString() })}</p>
    </div>
  )
}
```
...
```tsx
// Provide defaults for better development experience
t('feature.newKey', { defaultValue: 'Default text for new feature' })
```
...
This documentation provides a complete guide for implementing and extending the i18n system in your React frontend with TypeScript.